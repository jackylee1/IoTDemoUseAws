...
public class DBOperator{
	...
	public static GetItemOutcome getItem(String tblName, String hashKey){
		...
		GetItemOutcome outcome = table.getItemOutcome(new GetItemSpec()
                .withPrimaryKey(DBConstants.HASH_KEY_NAME, hashKey));
...
/*	private static Item newItem() {
		Item item = new Item().withString(HASH_KEY_NAME, "foo")
				.withBinary("binary", new byte[] { 1, 2, 3, 4 })
				.withBinarySet("binarySet", new byte[] { 5, 6 }, new byte[] { 7, 8 })
				.withBoolean("booleanTrue", true)
				.withBoolean("booleanFalse", false).withInt("intAttr", 1234).withList("listAtr", "abc", "123")
				.withMap("mapAttr", new ValueMap().withString("key1", "value1").withInt("key2", 999))
				.withNull("nullAttr").withNumber("numberAttr", 999.1234).withString("stringAttr", "bla")
